...
export function buildFallbackReading(chart, transit) {
  ...
  const transitLine = transit ? `Right now, ${transit.name} is in effect — ${transit.blurb} ` : "";
  return {
    cosmic_profile: `${chart.name} rises in ${chart.lagna} with the Moon in ${chart.moonSign} (${chart.nakshatra}). ${doshaLine}${transitLine}You are currently running the ${dasha} Mahadasha, which sets the tone for this chapter of life.`,
