...
let _audio = null;      // ONE reusable element, unlocked by a user gesture
let _curUrl = null;

function audioEl() {
  if (!_audio && typeof Audio !== "undefined") { _audio = new Audio(); _audio.preload = "auto"; }
  return _audio;
}

// Call this from a real user-gesture handler (click/tap). Browsers only allow
// programmatic audio playback that traces back to a recent gesture; Devi's
// replies can arrive 10–15s later (the agent loop), by which point a fresh
// Audio().play() is blocked and we fall back to the robotic browser voice.
// Priming a single reusable element here keeps it unlocked, so the premium voice
// plays no matter how long the reply takes.
...
async function speakEleven(text, eff, { onStart, onEnd } = {}) {
  ...
  const a = audioEl();
  if (!a) { URL.revokeObjectURL(url); throw new Error("no_audio"); }
  // Reuse the gesture-unlocked element (don't new Audio(), which isn't unlocked).
...
export function stopSpeaking() {
  ...
  // Pause but KEEP the element — nulling it would lose the gesture unlock and the
  // next reply would be blocked from autoplaying the premium voice.
  try { if (_audio) _audio.pause(); } catch { /* ignore */ }
}
