...
// Premium ElevenLabs voices (English + Hindi) via server proxy, with a graceful
// fallback to the browser's female voice per language.
const _elevenByLang = {}; // lang -> true (works) / false (unavailable)
let _audio = null;
...
export function speak(text, lang = "en", cb = {}) {
  ...
  if ((eff === "en" || eff === "hi") && _elevenByLang[eff] !== false) {
    speakEleven(t, eff, cb).catch((e) => {
      // Only DISABLE the premium voice for the session when it's genuinely not
      // configured/available on the server (503/501). Transient failures — a
      // blocked autoplay, a network blip, a temporary 5xx — fall back to the
      // browser voice for this one line but are retried on the next utterance,
      // so a single hiccup no longer permanently loses Devi's synthesized voice.
      const code = String((e && e.message) || "");
      if (code === "tts_503" || code === "tts_501") _elevenByLang[eff] = false;
      webSpeak(t, lang, cb);
    });
    return;
  }
