...
// Lightweight health check for diagnosing deploys (is the DB configured + reachable?).
export async function dbStatus() {
  if (!CONN) return { configured: false, ok: false, error: "POSTGRES_URL (or DATABASE_URL) is not set" };
  try { await getPool().query("SELECT 1"); return { configured: true, ok: true }; }
