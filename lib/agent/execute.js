...
import { pujasForIntent, templesForPuja } from "@/lib/data";
...
export async function executeTool(name, input = {}) {
  switch (name) {
    ...
    case "recommend_puja": {
      // Sourced recommendations from the Sacred Data Engine (verified rows only).
      let recs = pujasForIntent(input.intent || "");
      if (input.deity) {
        const d = String(input.deity).toLowerCase();
        const f = recs.filter((p) => (p.deity || []).some((x) => x.toLowerCase().includes(d)));
        if (f.length) recs = f;
      }
      if (!recs.length) {
        // No verified puja matches this intent yet — point to the full catalogue.
        return { result: { success: true, count: 0, intent: input.intent || null, note: "No verified pujas matched this intent yet; suggest browsing all pujas." }, action: { type: "navigate", href: "/pujas" } };
      }
      const recommendations = recs.slice(0, 4).map((p) => ({
        id: p.id,
        name: p.name?.common || p.id,
        significance: p.significance,
        disclaimer: p.disclaimer, // MUST be relayed
        offered_at: templesForPuja(p.id).map((t) => t.name),
      }));
      return {
        result: { success: true, count: recommendations.length, intent: input.intent || null, recommendations, note: "Relay each puja's disclaimer; frame significance as faith/tradition; never promise outcomes." },
        action: { type: "navigate", href: "/pujas" },
      };
    }
    default:
